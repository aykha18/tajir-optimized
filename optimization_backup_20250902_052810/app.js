...
document.addEventListener('DOMContentLoaded', function() {
  ...
  console.log('🔍 Navigation elements found:', {
    ...
    pages: pages.length,
    pages: Array.from(pages).map(p => ({ id: p.id, classes: p.className }))
  });

  navBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      console.log('🖱️ Navigation button clicked:', btn.dataset.go);
      ...
      // Hide all pages
      pages.forEach(p => {
        console.log(`👁️ Hiding page: ${p.id} (was: ${p.className})`);
        p.classList.add('hidden');
        console.log(`👁️ Page ${p.id} now has classes: ${p.className}`);
      });

      // Show target page
      const targetPage = document.getElementById(btn.dataset.go);
      if (targetPage) {
        console.log(`👁️ Showing target page: ${btn.dataset.go} (was: ${targetPage.className})`);
        targetPage.classList.remove('hidden');
        console.log(`👁️ Target page ${btn.dataset.go} now has classes: ${targetPage.className}`);
        // Update page title
