...
def add_security_headers(response):
    ...
    response.headers['Content-Security-Policy'] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.tailwindcss.com https://unpkg.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data:; connect-src 'self';"
        ...
        try:
            ...
            for type_suggestion in suggestions.get('product_types', []):
                ...
                if not existing:
                    cursor = conn.cursor()
                    cursor.execute(
                        'INSERT INTO product_types (user_id, type_name, description) VALUES (?, ?, ?)',
                ...
                for product_suggestion in type_suggestion.get('products', []):
                    ...
                    existing_product = conn.execute(
                        'SELECT product_id FROM products WHERE user_id = ? AND product_name = ?', 
                    ...
                    if not existing_product:
                        cursor = conn.cursor()
                        cursor.execute(
                            'INSERT INTO products (user_id, type_id, product_name, rate, description) VALUES (?, ?, ?, ?, ?)',
