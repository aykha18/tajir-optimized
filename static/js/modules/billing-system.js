...
// AI Voice Assistant Integration Methods
window.BillingSystem = {
  // Add product to bill from voice assistant
  addProductToBill: function(product, quantity) {
    try {
      // Find the product in the dropdown and select it
      const productSelect = document.getElementById('productSelect');
      if (productSelect) {
        // Find the option with matching product name
        const options = Array.from(productSelect.options);
        const matchingOption = options.find(option => 
          option.text.toLowerCase().includes(product.product_name.toLowerCase())
        );
        
        if (matchingOption) {
          productSelect.value = matchingOption.value;
          // Trigger change event to populate rate
          ...
          // Add to bill
          const addButton = document.getElementById('addToBillBtn');
          if (addButton) {
            addButton.click();
          }
          
          console.log(`Voice Assistant: Added ${quantity} ${product.product_name} to bill`);
        }
      }
    } catch (error) {
  ...
  // Select customer from voice assistant
  selectCustomer: function(customer) {
    try {
      ...
      if (mobileInput) mobileInput.value = customer.phone;
      if (cityInput) cityInput.value = customer.city;
      if (areaInput) areaInput.value = customer.area;
      
      // Trigger events to update any dependent fields
      [customerInput, mobileInput, cityInput, areaInput].forEach(input => {
        if (input) input.dispatchEvent(new Event('input'));
      });
      
      console.log(`Voice Assistant: Selected customer ${customer.name}`);
    } catch (error) {
      console.error('Error selecting customer:', error);
    }
  },

  // Create bill from voice assistant
  createBill: function() {
    try {
      const saveButton = document.getElementById('saveBillBtn');
      if (saveButton) {
        saveButton.click();
        console.log('Voice Assistant: Creating bill');
      }
    } catch (error) {
  ...
  // Print bill from voice assistant
  printBill: function() {
    try {
      const printButton = document.getElementById('printBillBtn');
      if (printButton) {
        printButton.click();
        console.log('Voice Assistant: Printing bill');
      }
    } catch (error) {
  ...
  // Clear bill from voice assistant
  clearBill: function() {
    try {
      const clearButton = document.getElementById('clearBillBtn');
      if (clearButton) {
        clearButton.click();
        console.log('Voice Assistant: Cleared bill');
      } else {
        // Fallback: clear form manually
        const form = document.getElementById('billingForm');
        if (form) {
          form.reset();
          // Clear bill items
          ...
          console.log('Voice Assistant: Cleared bill manually');
        }
      }
    } catch (error) {
